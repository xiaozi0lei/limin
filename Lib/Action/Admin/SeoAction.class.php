...
class SeoAction extends AdminInitAction {
	...
	public function link() {
		...
		$link_login=session('linklogin');
		
		if(empty($link_login)){
			
			
			$this->display ( './Public/Admin/Seo/linklogin.html' );
		}else{
	...
	public function doLinkLogin(){
		redirect(U('Admin/Seo/link'));
	...
	public function tpl(){
		...
		$tplname = $config_array ['pc_tmpl'];
		
		if(empty($_GET['edit'])){
			
			$this->display('./Public/Admin/Seo/tpl.html');
			return;
		}
		
		
		if(!empty($_GET['eidt'])&&!in_array($_GET['eidt'], array('header','index','footer'))){
			$this->error('不存在该模板');
		}else{
			
			if($this->isAjax()){
				if($_GET['edit']=='index'){
					$path='./Tpl/Home/'.$tplname.'/Index/index.html';
				}else{
					$path='./Tpl/Home/'.$tplname.'/Public/'.strtolower($_GET['edit']).'.html';
				}
			...
			$this->assign('tpl',$_GET['edit']);
			$this->display('./Public/Admin/Seo/edittpl.html');
		}
	...
	public function dosavetpl(){
		
		if(!in_array($_POST['tpl'], array('header','index','footer'))){
			$this->error('不存在该模板');
		}
		...
		if($_POST['tpl']=='index'){
				...
				$path='./Tpl/Home/'.$tplname.'/Public/'.strtolower($_POST['tpl']).'.html';
		}
