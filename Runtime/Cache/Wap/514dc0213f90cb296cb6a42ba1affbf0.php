...
function showmore(){
	...
	$.ajax({
	   type: "GET",
	   url: "<?php echo u('Wap/article/more');?>",
	   data: "count="+count+"&sid=<?php echo ($sort['id']); ?>",
	   dataType:'json',
...
<div data-role="header">
    <header class="headerNews">
      ...
      <span class="BigclassName"><?php if(!empty($keywords)): else: echo ($current_sort["name"]); endif; ?></span>
    ...
    <section>
    	  ...
    	  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><li class="items">
           <a href="<?php echo ($li["url"]); ?>">
            <div>
            <h3 class="newsTitle">
              <span class="name"><?php echo ($li["title"]); ?></span>
            </h3>
            <p class="time"><?php echo (date("y/m/d",$li['dateline'])); ?></p>
            </div>
            </a> 
	    ...
	    <?php if(empty($keywords)): ?><div class="more">
