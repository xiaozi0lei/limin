...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
    	...
    	'banners'   =>  array('level'=>3),
    	'allsorts'   =>  array('attr'=>'sortid,level','level'=>3,'close'=>0),
    	'sorts'   =>  array('attr'=>'sortid','level'=>3),
    	'items'   =>  array('attr'=>'sortid,limit','level'=>3),
    	'CustomItems'   =>  array('attr'=>'name,id,limit,where','level'=>3),
    ...
    public function _allsorts($attr,$content)
    {
    	$parsestr='';
    	//把标签的所有属性解析到$tag数组里面
    	...
    	$sortid=intval($tag['sortid']);
    	$level=intval($tag['level']);
    	$parsestr .='<?php $html=getAllSorts('.$sortid.','. $level.'); echo $html;?>';
    ...
    public function _sorts($attr,$content)
    {
    	$parsestr='';
    	//把标签的所有属性解析到$tag数组里面
    	$tag = $this->parseXmlAttr($attr,'category');
    	...
    	$sortid=intval($tag['sortid']);
    	$limit=intval($tag['limit']);
    	$istop=intval($tag['istop']);
    	$parsestr .='<?php $_list=getSorts('.$sortid.','. $limit.','. $istop.');';
    	$parsestr .='if(!empty($_list)){foreach($_list as $k=>$sort){?>'.
    			$parseStr .= $this->tpl->parse($content);
    	$parsestr .='<?php }}?>';
    ...
    public function _banners($attr=array(),$content)
    {
    	$parsestr='';
    	$parsestr .='<?php $_list=getBanner();';
    	$parsestr .='if(!empty($_list)){foreach($_list as $k=>$banner){?>'.
    			$parseStr .= $this->tpl->parse($content);
    	$parsestr .='<?php }}?>';
    ...
    public function _items($attr,$content)
    {
    	$parsestr='';
    	//把标签的所有属性解析到$tag数组里面
    	$tag = $this->parseXmlAttr($attr,'category');
    	...
    	$limit=intval($tag['limit']);
    	$istop=intval($tag['istop']);    	
    	$parsestr .='<?php $_list=getItems('.$sortid.','. $limit.','. $istop.');';
    	$parsestr .='if(!empty($_list)){foreach($_list as $k=>$item){?>'.
      	$parseStr .= $this->tpl->parse($content);
    	$parsestr .='<?php }}?>';
    ...
    public function _CustomItems($attr,$content)
    {
    	$parsestr='';
    	//把标签的所有属性解析到$tag数组里面
    	$tag = $this->parseXmlAttr($attr,'CustomItems');
    	...
    	switch ($name)
    	{
    		...
    		/* case '':
    			...
    			break;*/
    		default:
    			$parsestr = '暂时未开通此类别，请联系开发人员';
    			return $parsestr;
    	}
    	//return $parsestr = $where;
    	$limit=intval($tag['limit']); 
    	$where = trim($tag['where']);
    	$res = M("sort")->field("id")->where("lang= '$lang' and name = '$where' and module='$name'")->find();
    	if (empty($res))
    	{
    		$parsestr = '内容尚未录入或参数错误！！！';
    		return $parsestr;
    	}
    	$parsestr .='<?php $list ='. "M('".$name."')->where('sort=".$res['id']."')->limit(".$limit.')->order("top desc,dateline desc")->select();';

    	// 处理URL
    	$parsestr .='
    	if (! empty ( $list )) {
    		foreach ( $list as $k => $v ) {
    			$list [$k] [url] = listurl ('."'". $categroy."'".', array (
    					'."'".'sid'."'".' => $v [sort],
    					id => $v [id]
    			) );
    		}
    	...
    	$parsestr .='if(!empty($list)){foreach($list as $k=>$item){$'.$tag['id'].'= $item;?>';
    	/*$parsestr .='if(!empty($'.$tag['id'].')){foreach($'.$tag['id'].' as $k=>$item){?>';*/
    	$parsestr .= $this->tpl->parse($content);
    	$parsestr .='<?php }}?>';
    	return $parsestr;
    }
